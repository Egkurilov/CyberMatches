#!/usr/bin/env python3
"""
cs2.py — Liquipedia Counter-Strike/CS2 parser (Liquipedia:Matches)
...
from cybermatches.common.metrics import (
    record_parse_result,
...
from cybermatches.common.time import parse_time_to_target_tz
...
LOG_FILE = str(BASE_DIR / "logs" / "cs2_parser.log")
...
def _get_match_counts() -> tuple[int, int]:
    now_msk = datetime.now(TARGET_TZ)
    start_dt = datetime.combine(now_msk.date(), datetime.min.time(), tzinfo=TARGET_TZ)
    end_dt = start_dt + timedelta(days=1)

    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) FROM public.{MATCHES_TABLE};")
            (total,) = cur.fetchone()
            cur.execute(
                f"""
                SELECT COUNT(*)
...
def _parse_score_tuple(score: Optional[str]) -> Optional[Tuple[int, int]]:
    return parse_score_tuple(score, max_points=50)
...
def parse_matches_from_html(html: str) -> List[Match]:
    ...
    for c in containers:
        ...
        if time_msk is None:
            ...
            time_raw = time_el.get_text(" ", strip=True) if time_el else None
            time_msk = parse_time_to_target_tz(time_raw or "", TARGET_TZ, container=c, tz_map=TZ_IANA_MAP)
...
def worker_once() -> None:
    ...
    try:
        ...
        if os.getenv("SKIP_SCORE_UPDATE", "").lower() not in {"1", "true", "yes"}:
        ...
        try:
            total, today = _get_match_counts()
            update_counts("cs2", total, today)
        except Exception as e:
            logger.warning("Не удалось обновить метрики количества матчей: %s", e)

        record_parse_result(
            "cs2",
            success=True,
            duration_sec=elapsed,
            parsed_count=metrics["parsed_matches"],
            deduped_count=metrics["deduped_matches"],
        )
        ...
        record_parse_result(
            ...
            success=False,
            duration_sec=elapsed,
            parsed_count=metrics.get("parsed_matches"),
            deduped_count=metrics.get("deduped_matches"),
            error=str(e),
        )
        raise
