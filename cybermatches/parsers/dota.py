#!/usr/bin/env python3
"""
dota.py — парсер матчей Liquipedia с:
...
from cybermatches.common.time import parse_liquipedia_time, parse_time_to_msk
...
LOG_FILE = str(BASE_DIR / "logs" / "parser.log")
...
def _get_match_counts() -> tuple[int, int]:
    ...
    start_dt = datetime.combine(now_msk.date(), datetime.min.time(), tzinfo=MSK_TZ)
...
def parse_matches_from_html(html: str) -> List[Match]:
    ...
    for container in containers:
        ...
        if time_raw:
            dt_utc, parsed_msk = parse_liquipedia_time(time_raw, MSK_TZ, TZ_IANA_MAP)
            if parsed_msk is not None:
                time_msk = parsed_msk
            else:
                # более терпимый парсер (умеет жить с <abbr> и прочим)
                time_msk = parse_time_to_msk(time_raw, TZ_IANA_MAP)
...
def update_scores_with_retry(max_retries: int = 3) -> None:
    attempt = 1
    while True:
        try:
            ...
            logger.warning(
                "Deadlock при обновлении счёта (попытка %s/%s): %s",
                attempt,
                max_retries,
                e,
            )
...
def worker_once() -> None:
    ...
    try:
        ...
        try:
            ...
            update_counts("dota", total, today)
        except Exception as e:
